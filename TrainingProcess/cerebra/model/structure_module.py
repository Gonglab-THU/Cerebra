...
class StructureStack(nn.Module):
    ...
    def forward(self, x1D, x2D, AncherList):
        ...
        quaternion = torch.tensor([1., 0., 0., 0.])[None, None, None, :].repeat(batch, len(AncherList), length, 1).to(dtype=x1D.dtype)
        translation = torch.tensor([0., 0., 0.])[None, None, None, :].repeat(batch, len(AncherList), length, 1).to(dtype=x1D.dtype)
        ...
        pLDDT = torch.sigmoid(self.pLDDT(x2D))
