...
from cerebra_model.humodel.li_ops import LieOps
eps = 1e-5
...
class X1D_UpDate_X2D(nn.Module):
    ...
    def forward(self, x1D, x2D):
        ...
        atten1 = torch.where(torch.isinf(atten1), torch.full_like(atten1, 65500),atten1)
        ...
        # print('msa',atten_MSA)
        # print('target',atten_target)
        # print('x2D',x2D)
        X2D_Updated = self.UpdateX2D(torch.cat((atten_MSA, atten_target, x2D), dim=1))
        # print('x2D',X2D_Updated)
        X2D_Updated = torch.where(torch.isnan(X2D_Updated), torch.full_like(X2D_Updated, 0),X2D_Updated)
        # print('x2D_1',X2D_Updated)
...
def AlignQuaternionSign(q, reference):
    """Choose the quaternion lift closest to a reference quaternion."""
    return torch.where(
        (q * reference).sum(dim=-1, keepdim=True) < 0,
        -q,
        q,
    )


def CanonicalQuaternion(q):
    """Normalize an output quaternion and use the non-negative-real lift."""
...
class PathSynthesis(nn.Module):
    ...
    def TransformQ(self, quaternion, AncherList, affinity):
        ...
        # 1. Use each anchor's self-pair as its tangent-space origin.
        # q_base: [head, batch, k, 1, length, 4]
        idx = torch.arange(k, device=quaternion.device)
        q_base = q_step1[:, :, idx, idx, :, :].unsqueeze(3)
        ...
        # Align candidates to the synthesized quaternion before measuring the
        # Euclidean residual, avoiding a false q versus -q penalty.
        q_final_detached = q_final.unsqueeze(3).detach()
        q_aligned_for_loss = AlignQuaternionSign(q_aligned, q_final_detached)
        quaternion_est = q_aligned_for_loss - q_final_detached
        quaternion_est = (quaternion_est ** 2).sum(-1).mean(dim=(0, 3))

        return q_final, quaternion_est
    ...
    def TransformT(self, translation, quaternion, AncherList, affinity):
        ...
        translation_est = t_step1 - t.mean(0).unsqueeze(0).unsqueeze(3).detach()
    ...
    def forward(self, x2D, quaternion, translation, AncherList):
        ...
        # Align the quaternion lifts before averaging prediction heads.
        quaternion = AlignQuaternionSign(quaternion, quaternion[:1])
        quaternion = F.normalize(quaternion.mean(dim=0), dim=-1)
...
class StructureBlock(nn.Module):
    ...
    def forward(self, x2D, quaternion, translation, AncherList):
        ...
        if seq_length >= 0:
            ...
            if (quaternion - Ind_quaternion).abs().sum() > 1e-3:
                new_quaternion = AlignQuaternionSign(new_quaternion, quaternion)
                new_quaternion = F.normalize(quaternion + new_quaternion, dim=-1)
            return x2D, new_quaternion, new_translation, quaternion_est, translation_est
        else:
            new_translation = new_translation.mean(dim=0)

            new_quaternion = AlignQuaternionSign(new_quaternion, new_quaternion[:1])
            new_quaternion = F.normalize(new_quaternion.mean(dim=0), dim=-1)
            if (quaternion - Ind_quaternion).abs().sum() > 1e-3:
                new_quaternion = AlignQuaternionSign(new_quaternion, quaternion)
                new_quaternion = F.normalize(quaternion + new_quaternion, dim=-1)
...
class StructureStack(nn.Module):
    def __init__(self):
        ...
        #     nn.Dropout(p=drouout),
        #     nn.InstanceNorm2d(dim_x2D),
        #     Conv2d(dim_x2D, 64, kernel_size=3, padding=1),
        # )
    
    def forward(self, x1D, x2D, AncherList, keep_all=True):
        ...
        for s in [self.S1, self.S2, self.S3, self.S4, self.S5, self.S6]:
            ...
            if keep_all:
                ...
                collection_quaternion.append(CanonicalQuaternion(quaternion))
            ...
            #     pLDDT = torch.sigmoid(self.pLDDT(x2D.detach()))
                # pAE = self.pAE(x2D.detach())
        if not keep_all:
            collection_translation = [translation]
            collection_quaternion = [CanonicalQuaternion(quaternion)]
            collection_translation_est = [torch.sqrt(translation_est + 1e-6).mean()]
            collection_quaternion_est = [torch.sqrt(quaternion_est + 1e-6).mean()]
        return x1D, x2D, collection_translation, collection_quaternion, collection_translation_est, collection_quaternion_est
