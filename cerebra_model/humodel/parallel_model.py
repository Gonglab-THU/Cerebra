...
from cerebra_model.humodel.structure_module_new import StructureStack_parallel,AngleResnet
...
class AlphaFold(nn.Module):
    ...
    def __init__(self, config,device_list=["cuda:0","cuda:1","cuda:2","cuda:3"]):
        ...
        if device_num == 4:
            ...
            self.device0, self.device1, self.device2, self.device3 = (torch.device(device_list[0]), torch.device(device_list[0]), torch.device(device_list[1]), torch.device(device_list[2]))
        elif device_num == 2:
        ...
        self.esm2 = Linear(2560, 256, init="relu").to(self.device0) 
        
        self.evoformer = EvoformerStack(
            **self.config["evoformer_stack"],
        ).to(self.device1) 

        self.structure = StructureStack_parallel(device_list=["cuda:0","cuda:1"])

        self.dist36bin = Dist36bin(128).to(self.device1)
        ...
        self.bert = nn.Linear(256, 23).to(self.device1)
        ...
        self.angle_resnet = AngleResnet(256).to(self.device3)
    def forward(self, feats, prevs, AncherList, _recycle=True):
        ...
        # Prep some features
        seq_mask = feats["seq_mask"].to(self.device0)
        pair_mask = seq_mask[..., None] * seq_mask[..., None, :].to(self.device0)
        msa_mask = feats["msa_mask"].to(self.device0)
        
        for k,v in feats.items():
            feats[k] = v.to(self.device0)
        ...
        angles = [unnormalized_angles.to(self.device3), angles.to(self.device3)]
        ...
        outputs['bert'] = self.bert(m).to(self.device3)
        ...
        outputs['pLDDT'] = pLDDT.to(self.device3)
        outputs['CE'] = CE.to(self.device3)
        outputs['PsiPhi'] = PsiPhi.to(self.device3)
