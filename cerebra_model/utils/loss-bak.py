...
from cerebra_model.utils import feats
from cerebra_model.utils.rigid_utils import Rotation, Rigid
...
def between_residue_bond_loss(
    ...
    gt_length = gt_length.to(c_n_bond_length.device)
    ...
    mask = (this_ca_mask * this_c_mask * next_n_mask * has_no_gap_mask).to(c_n_bond_length.device)
    ...
    mask = (this_c_mask * next_n_mask * next_ca_mask * has_no_gap_mask).to(c_n_bond_length.device)
...
def between_residue_clash_loss(
    ...
    dists_mask = (dists_mask * (1.0 - disulfide_bonds)).to(atom14_atom_radius.device)
...
def within_residue_violations(
    ...
    dists_masks = dists_masks.to(dists_to_low_error.device)
...
def find_structural_violations(
    ...
    **kwargs,
    ...
    atom14_atom_radius = (
        batch["atom14_atom_exists"].to(atomtype_radius[batch["residx_atom14_to_atom37"]].device)
...
def compute_renamed_ground_truth(
    ...
    renamed_atom14_gt_positions = (1.0 - alt_naming_is_better[..., None, None]) * atom14_gt_positions.to(alt_naming_is_better.device) + alt_naming_is_better[..., None, None] * atom14_alt_gt_positions.to(alt_naming_is_better.device)

    renamed_atom14_gt_mask = (
        1.0 - alt_naming_is_better[..., None]
    ) * atom14_gt_exists.to(alt_naming_is_better.device) + alt_naming_is_better[..., None] * batch["atom14_alt_gt_exists"].to(alt_naming_is_better.device)
