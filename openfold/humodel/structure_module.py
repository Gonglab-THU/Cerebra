...
def weightQuaternions(collection_q_avg, atten_Q, atten_dim=2):
    collection_q_avg = NormQuaternion(collection_q_avg)
    A = collection_q_avg.unsqueeze(-1) @ collection_q_avg.unsqueeze(-2)
    A = (A * (atten_Q.unsqueeze(-1))).sum(atten_dim)
    eigenValues, eigenVectors = torch.linalg.eig(A)
    eigenValues, eigenVectors = eigenValues.real, eigenVectors.real
    index = eigenValues.argsort(descending=True, dim=-1)[..., 0]
    t = index.dim() * [1]
    t.extend([4, 1])
    index = index[..., None, None].repeat(t)
    q2 = eigenVectors.gather(eigenVectors.dim()-1, index).squeeze(-1)
    return NormQuaternion(q2)
...
class PathSynthesis(nn.Module):
    ...
    def TransformQ(self, quaternion, AncherList, affinity):
        ...
        q = (atten * q_step1).sum(3)
        # q = NormQuaternion(weightQuaternions(q_step1, atten, 3))

        quaternion_est = q_step1 - q.unsqueeze(3).detach()
        # quaternion_est = (quaternion_est * quaternion_est).sum(-1).mean(dim=(0, 3))
        quaternion_est = quaternion_est.abs().sum(-1).mean(dim=(0, 3))
    ...
    def TransformT(self, translation, quaternion, AncherList, affinity):
        ...
        translation_est = t_step1 - t.unsqueeze(3).detach()
    ...
    def forward(self, x2D, quaternion, translation, AncherList):
        ...
        # quaternion = NormQuaternion(weightQuaternions(quaternion, torch.ones_like(quaternion)/4., 0))
...
class StructureBlock(nn.Module):
    ...
    def forward(self, x2D, quaternion, translation, AncherList, num):
        ...
        raw_translation = new_translation
        ...
        if seq_length >= 0:
            ...
            if (quaternion - Ind_quaternion).abs().sum() > 1e-3:
                ...
                # new_quaternion = NormQuaternion(weightQuaternions(torch.stack([quaternion + new_quaternion]), torch.ones_like(quaternion)/2., 0))
...
class StructureModule(nn.Module):
    ...
    def forward(self, x1D, x2D, quaternion, translation, AncherList, num):
        x1D, x2D, quaternion, translation = x1D, x2D, quaternion, translation
        x2D = self.X1D_UpDate_X2D(x1D, x2D)
        x2D, quaternion, translation, quaternion_est, translation_est = self.StructureBlock(x2D, quaternion, translation, AncherList, num)
...
class StructureStack(nn.Module):
    ...
    def forward(self, x1D, x2D, AncherList):
        ...
        for s in [self.S1, self.S2, self.S3, self.S4, self.S5, self.S6]:
            num += 1
            x1D, x2D, quaternion, translation, quaternion_est, translation_est = s(x1D, x2D, quaternion, translation, AncherList, num)
